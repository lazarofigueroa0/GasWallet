// server.js — GasWallet v3
// KEY CHANGE: This server now serves the frontend HTML directly.
// Cashiers just open a browser and go to http://YOUR-IP:3000
// No files to copy, no setup on their device.

const express = require("express");
const cors    = require("cors");
const path    = require("path");
const { v4: uuidv4 } = require("uuid");
const db      = require("./db");

const app  = express();
const PORT = process.env.PORT || 3000; // Uses $PORT env var on cloud, 3000 locally

app.use(cors());
app.use(express.json());

// ── Serve the frontend ────────────────────────────────────────────────────────
// This single line makes Express serve index.html (and any other static files)
// from the "public" folder inside this backend directory.
// Cashiers open: http://YOUR-SERVER-IP:3000
app.use(express.static(path.join(__dirname, "public")));


// ═══════════════════════════════════════════════════════════════════════════════
// CASHIER ROUTES
// ═══════════════════════════════════════════════════════════════════════════════

app.post("/cashiers/login", (req, res) => {
  const { name, pin } = req.body;
  if (!name || !pin) return res.status(400).json({ error: "Name and PIN required" });

  const cashier = db
    .prepare("SELECT * FROM cashiers WHERE LOWER(name) = LOWER(?) AND pin = ?")
    .get(name.trim(), pin.trim());

  if (!cashier) return res.status(401).json({ error: "Invalid name or PIN" });
  // Return id, name AND role — frontend uses role to show/hide tabs
  return res.json({ id: cashier.id, name: cashier.name, role: cashier.role });
});

app.get("/cashiers", (req, res) => {
  const cashiers = db.prepare("SELECT id, name FROM cashiers ORDER BY name").all();
  return res.json(cashiers);
});

app.post("/cashiers", (req, res) => {
  const { name, pin, role = "cashier" } = req.body;
  if (!name || !pin) return res.status(400).json({ error: "Name and PIN required" });
  if (pin.length < 4) return res.status(400).json({ error: "PIN must be at least 4 digits" });
  if (!["cashier","manager"].includes(role)) return res.status(400).json({ error: "Role must be cashier or manager" });

  const exists = db.prepare("SELECT id FROM cashiers WHERE LOWER(name) = LOWER(?)").get(name.trim());
  if (exists) return res.status(400).json({ error: "A cashier with that name already exists" });

  const result = db.prepare("INSERT INTO cashiers (name, pin, role) VALUES (?, ?, ?)").run(name.trim(), pin.trim(), role);
  return res.status(201).json({ id: result.lastInsertRowid, name: name.trim(), role });
});


// ═══════════════════════════════════════════════════════════════════════════════
// USER ROUTES
// ═══════════════════════════════════════════════════════════════════════════════

app.post("/users", (req, res) => {
  const { name } = req.body;
  if (!name || !name.trim()) return res.status(400).json({ error: "Name is required" });

  const id = uuidv4();
  db.prepare("INSERT INTO users (id, name, balance) VALUES (?, ?, 0)").run(id, name.trim());
  return res.status(201).json(db.prepare("SELECT * FROM users WHERE id = ?").get(id));
});

// GET /users — list all users, optionally filter by ?search=name
app.get("/users", (req, res) => {
  const { search } = req.query;
  let users;
  if (search && search.trim()) {
    users = db.prepare(`
      SELECT * FROM users
      WHERE LOWER(name) LIKE LOWER(?) OR id = ?
      ORDER BY name ASC
    `).all(`%${search.trim()}%`, search.trim());
  } else {
    users = db.prepare("SELECT * FROM users ORDER BY name ASC").all();
  }
  return res.json(users);
});

app.get("/users/:id", (req, res) => {
  const user = db.prepare("SELECT * FROM users WHERE id = ?").get(req.params.id);
  if (!user) return res.status(404).json({ error: "User not found" });
  return res.json(user);
});


// ═══════════════════════════════════════════════════════════════════════════════
// TRANSACTION ROUTES
// ═══════════════════════════════════════════════════════════════════════════════

app.post("/topup/preview", (req, res) => {
  const { userId, amount, cashierId } = req.body;
  if (!userId || !amount || !cashierId) return res.status(400).json({ error: "userId, amount and cashierId are required" });
  if (amount <= 0) return res.status(400).json({ error: "Amount must be greater than 0" });

  const user    = db.prepare("SELECT * FROM users WHERE id = ?").get(userId);
  const cashier = db.prepare("SELECT id, name FROM cashiers WHERE id = ?").get(cashierId);

  if (!user)    return res.status(404).json({ error: "User not found" });
  if (!cashier) return res.status(404).json({ error: "Cashier not found" });

  return res.json({
    user:      { name: user.name, balance: user.balance },
    cashier:   { name: cashier.name },
    amount,
    newBalance: +(user.balance + amount).toFixed(2),
  });
});

app.post("/topup", (req, res) => {
  const { userId, amount, cashierId } = req.body;
  if (!userId || !amount || !cashierId) return res.status(400).json({ error: "userId, amount and cashierId are required" });
  if (amount <= 0) return res.status(400).json({ error: "Amount must be greater than 0" });

  const user    = db.prepare("SELECT * FROM users WHERE id = ?").get(userId);
  const cashier = db.prepare("SELECT id FROM cashiers WHERE id = ?").get(cashierId);

  if (!user)    return res.status(404).json({ error: "User not found" });
  if (!cashier) return res.status(404).json({ error: "Cashier not found" });

  let transactionId;
  const doTopup = db.transaction(() => {
    db.prepare("UPDATE users SET balance = balance + ? WHERE id = ?").run(amount, userId);
    const result = db.prepare(
      "INSERT INTO transactions (user_id, cashier_id, type, amount) VALUES (?, ?, 'topup', ?)"
    ).run(userId, cashierId, amount);
    transactionId = result.lastInsertRowid;
  });

  doTopup();

  const updated = db.prepare("SELECT balance FROM users WHERE id = ?").get(userId);
  return res.json({ message: "Top-up successful", newBalance: updated.balance, transactionId });
});

app.post("/pay", (req, res) => {
  const { userId, amount, cashierId } = req.body;
  if (!userId || !amount || !cashierId) return res.status(400).json({ error: "userId, amount and cashierId are required" });
  if (amount <= 0) return res.status(400).json({ error: "Amount must be greater than 0" });

  const user    = db.prepare("SELECT * FROM users WHERE id = ?").get(userId);
  const cashier = db.prepare("SELECT * FROM cashiers WHERE id = ?").get(cashierId);

  if (!user)    return res.status(404).json({ error: "User not found" });
  if (!cashier) return res.status(404).json({ error: "Cashier not found" });
  if (cashier.role === "manager") return res.status(403).json({ error: "Managers cannot process payments" });

  if (user.balance < amount) {
    return res.status(400).json({
      error: "Insufficient balance",
      currentBalance: user.balance,
      required: amount,
      shortfall: +(amount - user.balance).toFixed(2),
    });
  }

  // 2% cashback on every payment, capped at $1.00
  const CASHBACK_RATE = 0.02;
  const CASHBACK_CAP  = 1.00;
  const cashback = +( Math.min(amount * CASHBACK_RATE, CASHBACK_CAP) ).toFixed(2);

  let transactionId;
  const doPayment = db.transaction(() => {
    // Deduct payment
    db.prepare("UPDATE users SET balance = balance - ? WHERE id = ?").run(amount, userId);
    const result = db.prepare(
      "INSERT INTO transactions (user_id, cashier_id, type, amount) VALUES (?, ?, 'payment', ?)"
    ).run(userId, cashierId, amount);
    transactionId = result.lastInsertRowid;

    // Apply cashback instantly
    db.prepare("UPDATE users SET balance = balance + ? WHERE id = ?").run(cashback, userId);
    db.prepare(
      "INSERT INTO transactions (user_id, cashier_id, type, amount) VALUES (?, ?, 'cashback', ?)"
    ).run(userId, cashierId, cashback);
  });

  doPayment();

  const updated = db.prepare("SELECT balance FROM users WHERE id = ?").get(userId);
  return res.json({
    message: "Payment successful",
    newBalance: updated.balance,
    transactionId,
    cashback, // send to frontend so receipt can show it
  });
});

app.get("/transactions/:userId", (req, res) => {
  const user = db.prepare("SELECT id FROM users WHERE id = ?").get(req.params.userId);
  if (!user) return res.status(404).json({ error: "User not found" });

  const txs = db.prepare(`
    SELECT t.*, c.name AS cashier_name
    FROM transactions t
    JOIN cashiers c ON c.id = t.cashier_id
    WHERE t.user_id = ?
    ORDER BY t.timestamp DESC
  `).all(req.params.userId);

  return res.json(txs);
});

app.get("/receipt/:transactionId", (req, res) => {
  const tx = db.prepare(`
    SELECT t.*, u.name AS user_name, c.name AS cashier_name
    FROM transactions t
    JOIN users    u ON u.id = t.user_id
    JOIN cashiers c ON c.id = t.cashier_id
    WHERE t.id = ?
  `).get(req.params.transactionId);

  if (!tx) return res.status(404).json({ error: "Transaction not found" });

  const user = db.prepare("SELECT balance FROM users WHERE id = ?").get(tx.user_id);

  // If this was a payment, find the cashback transaction that immediately followed
  let cashback = 0;
  if (tx.type === "payment") {
    const cbTx = db.prepare(`
      SELECT amount FROM transactions
      WHERE user_id = ? AND type = 'cashback' AND id > ?
      ORDER BY id ASC LIMIT 1
    `).get(tx.user_id, tx.id);
    if (cbTx) cashback = cbTx.amount;
  }

  return res.json({
    receiptNumber:  tx.id,
    type:           tx.type,
    amount:         tx.amount,
    cashback,
    timestamp:      tx.timestamp,
    customer:       { name: tx.user_name },
    cashier:        { name: tx.cashier_name },
    currentBalance: user.balance,
    station:        process.env.STATION_NAME || "GasWallet Station",
  });
});


// ═══════════════════════════════════════════════════════════════════════════════
// SUMMARY ROUTE
// ═══════════════════════════════════════════════════════════════════════════════

/**
 * GET /summary
 * Returns all key metrics for the dashboard in one call.
 */
app.get("/summary", (req, res) => {
  // Total cash held across all users
  const { totalCash } = db.prepare("SELECT COALESCE(SUM(balance),0) AS totalCash FROM users").get();

  // User counts
  const { totalUsers }    = db.prepare("SELECT COUNT(*) AS totalUsers FROM users").get();
  const { activeUsers }   = db.prepare("SELECT COUNT(*) AS activeUsers FROM users WHERE balance > 0").get();
  const { inactiveUsers } = db.prepare("SELECT COUNT(*) AS inactiveUsers FROM users WHERE balance = 0").get();
  const { lowBalance }    = db.prepare("SELECT COUNT(*) AS lowBalance FROM users WHERE balance > 0 AND balance < 10").get();

  // Today's transactions (SQLite date in UTC)
  const todayStats = db.prepare(`
    SELECT
      COALESCE(SUM(CASE WHEN type='topup'    THEN amount ELSE 0 END),0) AS topupsToday,
      COALESCE(SUM(CASE WHEN type='payment'  THEN amount ELSE 0 END),0) AS paymentsToday,
      COALESCE(SUM(CASE WHEN type='cashback' THEN amount ELSE 0 END),0) AS cashbackToday,
      COUNT(CASE WHEN type='payment' THEN 1 END) AS txCountToday
    FROM transactions
    WHERE DATE(timestamp) = DATE('now')
  `).get();

  // Top 5 customers by balance
  const topCustomers = db.prepare(`
    SELECT id, name, balance FROM users
    WHERE balance > 0
    ORDER BY balance DESC LIMIT 5
  `).all();

  // Low balance customers (>0 but <10)
  const lowBalanceCustomers = db.prepare(`
    SELECT id, name, balance FROM users
    WHERE balance > 0 AND balance < 10
    ORDER BY balance ASC LIMIT 5
  `).all();

  // Cashier activity today
  const cashierActivity = db.prepare(`
    SELECT c.name, COUNT(*) AS txCount,
      COALESCE(SUM(CASE WHEN t.type='topup'   THEN t.amount ELSE 0 END),0) AS topups,
      COALESCE(SUM(CASE WHEN t.type='payment' THEN t.amount ELSE 0 END),0) AS payments
    FROM transactions t
    JOIN cashiers c ON c.id = t.cashier_id
    WHERE DATE(t.timestamp) = DATE('now')
    GROUP BY c.id ORDER BY txCount DESC
  `).all();

  // Recent transactions (last 5)
  const recentTx = db.prepare(`
    SELECT t.*, u.name AS user_name, c.name AS cashier_name
    FROM transactions t
    JOIN users u ON u.id = t.user_id
    JOIN cashiers c ON c.id = t.cashier_id
    ORDER BY t.timestamp DESC LIMIT 5
  `).all();

  // Total cashback ever given out
  const { totalCashbackEver } = db.prepare(
    "SELECT COALESCE(SUM(amount),0) AS totalCashbackEver FROM transactions WHERE type='cashback'"
  ).get();

  return res.json({
    totalCash, totalUsers, activeUsers, inactiveUsers, lowBalance,
    ...todayStats,
    totalCashbackEver,
    topCustomers, lowBalanceCustomers, cashierActivity, recentTx,
  });
});

// ── Start ─────────────────────────────────────────────────────────────────────
app.listen(PORT, "0.0.0.0", () => {
  // "0.0.0.0" means it listens on ALL network interfaces,
  // so other devices on the same WiFi can connect.
  console.log(`\n⛽  GasWallet v3 is running!`);
  console.log(`\n   Local:    http://localhost:${PORT}`);
  console.log(`   Network:  http://YOUR-IP:${PORT}  (share this with cashiers)\n`);
  console.log(`   To find YOUR-IP:`);
  console.log(`     Mac/Linux: run "ifconfig" and look for inet under en0`);
  console.log(`     Windows:   run "ipconfig" and look for IPv4 Address\n`);
});
